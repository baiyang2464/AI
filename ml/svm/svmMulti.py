...
#SVM class:train model and get predict accuracy
class SVM:
    ...
    def innerL(self,i, _oS):
        ...
        if ((_oS.labelMat[i]*Ei < -_oS.tol) and (_oS.alphas[i] < _oS.C)) or ((_oS.labelMat[i]*Ei > _oS.tol) and (_oS.alphas[i] > 0)):
            ...
            if (abs(_oS.alphas[j] - alphaJold) < 0.00001): return 0
    ...
    def getPredicton(self):
        ...
        m,n = shape(datMat)
        print">>>_oS has %d models"%(len(self._oS))
        for i in range(m):
            predicts=[]
            for num in range(0,10):
                kernelEval = self.kernelTrans(self._oS[num].sVs,datMat[i,:],self._kTup)
                predict=kernelEval.T * multiply(self._oS[num].labelSV,self._oS[num].alphas[self._oS[num].svInd]) + self._oS[num].b
                predicts.append(predict)
            predict = self.returnMaxNo(predicts)
            if predict == num:successCount +=1
            else:errorCount += 1    
        print "the test error rate is: %f" % (float(errorCount)/m) 
        print "the test success rate is: %f" % (float(successCount)/m) 
...
def main():
    ...
    toler = 0.0001
    maxIter = 10000
    kTup=('rbf',55)#rbf  gamma=50 accuracy=0.88   70,0.87
    #kTup=('lin',100)#rbf  gamma=50 accuracy=0.88   70,0.87
