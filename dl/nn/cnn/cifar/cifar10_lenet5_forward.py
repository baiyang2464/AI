...
def bn(x):#批归一化
    ...
    axes = list(range(len(x.get_shape())-1))#x的维度
    #bn_mean,bn_var = tf.nn.moments(z_bn,axes)
    bn_mean,bn_var = tf.nn.moments(x,axes)
    #z_hat = (z_bn-bn_mean)/tf.sqrt(bn_var+epsilon)
    y = tf.nn.batch_normalization(x,bn_mean,bn_var,offset=0,scale=1,variance_epsilon=1e-5)
    return y
